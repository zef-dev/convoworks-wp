...
{
    public const TYPE_SMALL_SKILL_URL = 'small_skill_icon';
    public const TYPE_LARGE_SKILL_URL = 'large_skill_icon';
            ...
            if (!isset($mapping['time_propagated']) || empty($mapping['time_propagated'])) {
                ...
                if ($mapping['time_propagated'] < $platform_config['time_updated']) {
                    ...
                    $configChanged = $this->_platformPublishingHistory->hasPropertyChangedSinceLastPropagation(
                        $this->_serviceId,
                        $this->getPlatformId(),
                        PlatformPublishingHistory::AMAZON_ACCOUNT_LINKING_INFORMATION,
                        $accountLinkingData
                    ) || $this->_platformPublishingHistory->hasPropertyChangedSinceLastPropagation(
                        $this->_serviceId,
                        $this->getPlatformId(),
                        PlatformPublishingHistory::AMAZON_SELF_SIGNED_CERTIFICATE,
                        $platform_config['self_signed_certificate']
                    ) || $this->_platformPublishingHistory->hasPropertyChangedSinceLastPropagation(
                        $this->_serviceId,
                        $this->getPlatformId(),
                        PlatformPublishingHistory::AMAZON_ENDPOINT_SSL_CERTIFICATE_TYPE,
                        $platform_config['endpoint_ssl_certificate_type']
                    ) || $this->_platformPublishingHistory->hasPropertyChangedSinceLastPropagation(
                        $this->_serviceId,
                        $this->getPlatformId(),
                        PlatformPublishingHistory::AMAZON_MANIFEST,
                        $manifest
                    ) || $this->_platformPublishingHistory->hasPropertyChangedSinceLastPropagation(
                        $this->_serviceId,
                        $this->getPlatformId(),
                        PlatformPublishingHistory::AMAZON_INTERACTION_MODEL,
                        $model
                    );
                    if ($configChanged) {
                        $changesCount++;
    ...
    public function export()
    {
        ...
        $export = new \Convo\Core\Util\SimpleFileResource(
            $this->_serviceId . '-' . $this->getPlatformId() . '.json',
            'application/json',
        ...
        if (!empty($mediaItem)) {
            try {
                ...
                if (is_array($parsedUrl) && count($parsedUrl) > 1) {
                    $iconUrl = $mediaItem;
                } elseif (is_array($parsedUrl) && count($parsedUrl) === 1) {
                ...
                } elseif ($typeSkillIconUrl === self::TYPE_LARGE_SKILL_URL) {
