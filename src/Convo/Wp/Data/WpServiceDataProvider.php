...
class WpServiceDataProvider extends AbstractServiceDataProvider
{
    ...
    public function createRelease(iAdminUser $user, $serviceId, $platformId, $type, $stage, $alias, $versionId, $meta)
    {
        ...
        $this->_checkError($this->_wpdb->query($this->_checkPrepare($this->_wpdb->prepare(
            "INSERT INTO {$this->_wpdb->prefix}convo_service_releases
    ...
    public function addPlatformReleaseData(IAdminUser $user, $serviceId, $platformId, $releaseId, $data)
    {
        $this->_checkError($this->_wpdb->query(
            $this->_checkPrepare(
                $this->_wpdb->prepare(
                    "UPDATE {$this->_wpdb->prefix}convo_service_releases SET `platform_release_data` = '%s', `time_updated` = '%d' WHERE `service_id` = '%s' AND `release_id` = '%s'",
                    json_encode($data, JSON_PRETTY_PRINT),
                    time(),
                    $serviceId,
                    $releaseId
